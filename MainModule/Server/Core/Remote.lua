...
return function(Vargs, GetEnv)
	...
	server.Remote = {
		...
		Returnables = {
			...
			UpdateCapes = function(p: Player,args: {[number]: any})
				local playerData = Core.GetPlayer(p)
				local capes = args[1] or {};

				if type(capes) == "table" then
					playerData.Capes = capes
		...
		MakeLocal = function(p: Player, object: Instance, parent: string?, clone: boolean?)
			object.Parent = p:FindFirstChildOfClass("PlayerGui") or p
			task.wait(0.5)
