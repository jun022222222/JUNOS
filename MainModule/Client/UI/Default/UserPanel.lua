...
return function(data, env)
	...
	local window = UI.Make("Window", {
		Name  = "UserPanel";
		Title = "JUNOS";
		Icon = "rbxassetid://134748329047273"; --"rbxassetid://9705471958"; --"rbxassetid://7681088830"; --"rbxassetid://7681233602"; --"rbxassetid://7681048299";
		Size  = {465, 325};
		AllowMultiple = false;
		OnClose = function()
	...
	if window then
		...
		local capeTab = tabFrame:NewTab("Capes", {
			Text = "Capes"
		})
		...
		if data.Tab then
			...
			elseif string.lower(data.Tab) == "aliases" then
				aliasTab:FocusTab()
			elseif string.lower(data.Tab) == "capes" then
				capeTab:FocusTab()
		...
		do
			infoTab:Add("TextLabel", {
				Text = `JUNOS maintained system based on Adonis Adminstration system created by Sceleratis (Davey_Bones).\
				\
				(JUNOS Currently in development)\
				\
				Managed by @jun022222222\
				\
				- Contact Here-\
				Discord: jun022222222 | Github jun022222222\
				\
				Based on Adonis({require(client.Shared.Changelog)[5]:match("%[(v%d*).+%].+")})`;
				TextWrapped = true;
		...
		do
			...
			dFrame:Add("TextButton", {
				Text = `Donate (300{utf8.char(0xE002)})`;
				ToolTip = `perks include`;
				Size = UDim2.new(1, -10, 0, 40);
				Position = UDim2.new(0, 5, 0, 5);
				BackgroundTransparency = 0.5;
				BackgroundColor3 = Color3.fromRGB(231, 6, 141);
				OnClick = function()
					promptPurchase(true, 182517242) --497917601)
				end
			})
			...
			--[[local Frame = donorTab:Add("TextButton", {
				Text = "Donor System Currently disabled",
		...
		--// Capes tab
		do
			local Capes = playerData.Capes
			local getCapes
			local edit
			local capelist = capeTab:Add("ScrollingFrame", {
				Size = UDim2.new(1, -10, 1, -35),
				Position = UDim2.new(0, 5, 0, 5),
			...
			function getCapes()
				capelist:ClearAllChildren()
				
				local num = 0
				for i, v in Capes do
					local frame = capelist:Add("Frame", {
						Size = UDim2.new(1, -10, 0, 100),
						Position = UDim2.new(0, 5, 0, num*105),
						BackgroundTransparency = 0.3,
					})

					local image = frame:Add("TextLabel", {
						Text = "Image:",
						...
						Children = {
							...
							},
							{
								...
								Position = UDim2.new(0, 5, 0, 20),
								BackgroundTransparency = 1,
								TextTransparency = v.Image~=0 and 1 or 0
							}
						}
					})

					for i,v2 in ipairs({"Color", "Material"}) do
						frame:Add("TextLabel", {
							Text = `{v2}:`,
							Size = UDim2.new(0, 150, 0, 40),
							Position = UDim2.new(0, 80, 0 , (i-1)*45 + 7),
							TextYAlignment = Enum.TextYAlignment.Top,
							Children = {
								{
									Class = "TextLabel",
									...
									Text = v2=="Color" and "" or v.Material,
									BackgroundColor3 = if type(v.Color) == "table" then
										Color3.new(v.Color[1],v.Color[2],v.Color[3])
										else
										BrickColor.new(v.Color).Color,
									BackgroundTransparency = v2=="Color" and 0 or 1
								}
							}
						})
					end
					...
					frame:Add("TextButton", {
						...
						OnClicked = function(new)
							playerData.Donor.Cape = v
							new.Text = Remote.Get("UpdateDonor", playerData.Donor)
							
							task.delay(0.029, function()
								window:Close()
								UI.Make("UserPanel", {Tab = "Capes"})
							end)
						end,
					})
					num += 1
				...
				capelist:ResizeCanvas(false, true)
			end
			
			function edit(key)
				if UI.Get("Cape Editor") then return end
				
				local data = key and Capes[key] or {}
				
				local currentColor = data.Color or Color3.new(1, 1, 1)
				local currentMaterial = data.Material or "Neon"
				local currentTexture = data.Image or 0
				...
				local function updateView()
					update = true
					capeview.Orientation = Vector3.new(0,0,0)
					
					capeview.Material = currentMaterial
					capeview.Color = currentColor
					capeview.Decal.Texture = `rbxassetid://{currentTexture}`
					task.spawn(function()
						task.wait()
						update = false
						while not update do
							capeview.Orientation += Vector3.new(0,1,0)
							task.wait(0.01)
						end
					end)
				end
				
				local editor = client.UI.Make("Window", {
					Title = "Cape Editor",
					Name = "Cape Editor",
					AllowMultiple = false,
					Size = {270, 400},
				...
				viewer = editor:Add("ViewportFrame", {
					Size = UDim2.new(0.5, -10, 1, -40);
					Position = UDim2.new(0, 5, 0, 5)
				})
				
				capeview = viewer:Add("Part", {
					Size = Vector3.new(2, 4, 0.1),
					Position = Vector3.new(0, 0, 0),
					Material = currentMaterial or "Neon",
					Children = {
						{
							Class = "BlockMesh",
				...
				cam = viewer:Add("Camera", {
					Parent = viewer,
					CFrame = CFrame.new(Vector3.new(0,0,5), capeview.Position)
				})
				
				viewer.CurrentCamera = cam
				
				editor:Add("Frame", {
					Size = UDim2.new(0, 5, 1, -40),
					Position = UDim2.new(0.5, 0, 0, 5)
				})
					
				editor:Add("TextLabel", {
					Text = "Cape Color:",
					...
					Children = {
						{
							...
							Name = "ColorPick",
							OnClicked = function(new)
								service.Debounce("CapeColorSet", function()
									local color = client.UI.Make("ColorPicker", {
										Color = currentColor
									})
						...
						},
						{
							...
							OnClicked = function(new)
								service.Debounce("CapeColorSet", function()
									local Brickcolor = UI.Make("ColorChooser")
									
									if Brickcolor then 
										currentColor = BrickColor.new(Brickcolor).Color
										new.Parent.ColorPick.BackgroundColor3 = currentColor
										updateView()
									else
										return
									end
				...
				editor:Add("TextLabel", {
					Text = "Cape Material:",
					Size = UDim2.new(0.5, -15, 0, 40),
					Position = UDim2.new(0.5, 10, 0, 50),
					TextYAlignment = Enum.TextYAlignment.Top,
					Children = {
						{
							Class = "Dropdown";
							...
							OnSelect = function(selection)
								service.Debounce("CapeMaterial", function()
									currentMaterial = selection
									updateView()
								end)
							end;
						}
				...
				editor:Add("TextLabel", {
					Text = "Cape Texture:",
					Size = UDim2.new(0.5, -15, 0, 40),
					Position = UDim2.new(0.5, 10, 0, 95),
					TextYAlignment = Enum.TextYAlignment.Top,
					Children = {
						{
							Class = "TextButton",
							...
							Position = UDim2.new(0, 5, 1, -20),
							OnClicked = function(new)
								service.Debounce("CapeTexture", function()
									local lastValid = currentTexture or 0
									local donePreview = false
									local pWindow = UI.Make("Window", {
										Name = "CapeTexture";
										Title = "Texture Preview";
				...
				editor:Add("TextButton", {
					Text = "Cancel",
					Size = UDim2.new(0.33, -5, 0, 20),
					Position = UDim2.new(0, 5, 1, -25),
					OnClicked = function()
						editor:Close()
					end,
				...
				if key then
					editor:Add("TextButton", {
						...
						OnClick = function()
							playerData.Capes[key] = nil
							Remote.Get("UpdateCapes", playerData.Capes)
							getCapes()
							editor:Close()
						end,
					})
				...
				save = playerData.isDonor and editor:Add("TextButton", {
					Text = "Save",
					Size = UDim2.new(0.33, -5, 0, 20),
					Position = UDim2.new(0.67, 0, 1, -25),
					OnClicked = function(new)
						playerData.Capes[key and key or Functions.GetRandom(8)] = {
