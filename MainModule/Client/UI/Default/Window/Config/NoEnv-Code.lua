...
return function(data, env)
	...
	function Expand(ent, point, text, richText)
		...
		if label then
			...
			ent.MouseMoved:Connect(function(x,y)
				...
				local sizeX, sizeY = bounds.X + 11, bounds.Y + 10
	...
	function create(class, dataFound, existing)
		...
		if dataFound then
			...
			if not data.NoCorner and new:IsA("GuiBase") and not new:IsA("ScrollingFrame") then
				create("UICorner", {
					Parent = new
				})
			end
	...
	if Icon then
		Iconf.Visible = true
		Iconf.Image = Icon
		Iconf.Size = UDim2.new(0, 25, 0, 25)
		Iconf.Position = UDim2.new(0, 3, 0, 3)
	...
	if Main and Main.UIColor then
		Event(service.RunService.RenderStepped, function() Main.UIColor.Rotation = (Main.UIColor.Rotation+0.5)%360 end)
	else
		print("UI Spinning Disabled")
	end
